    ...
    <label className="grid gap-2">
      ...
      {multiline ? (
        ...
        rows={2}
          ...
          className="w-full rounded-xl border border-zinc-800 bg-black px-3 py-2.5 text-sm text-zinc-100 outline-none transition placeholder:text-zinc-600 focus:border-zinc-600"
        />
      ) : (
        <input
          value={value}
            ...
            <ControlledDetails key={editorial.sectionId} className="rounded-2xl border border-zinc-800 bg-black/35 p-3" defaultOpen={index === 0}>
              ...
              <div className="mt-3 border-t border-zinc-800/80 pt-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <span className="text-xs text-zinc-500">Bölüm ayarları</span>
...
export function PageBlockEditor({ blocks, onChange, mediaAssets, locale, slug }: PageBlockEditorProps) {
  ...
  return (
    <section className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-4">
      ...
      <details className="mt-4 rounded-2xl border border-zinc-800 bg-black/30 p-3">
        <summary className="cursor-pointer list-none text-[11px] font-medium uppercase tracking-[0.18em] text-zinc-500 [&::-webkit-details-marker]:hidden">
          Sayfa şablonları
        </summary>
        <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-4">
          {templates.map((template) => (
            <button
              ...
              className="rounded-xl border border-zinc-800 bg-zinc-950/70 p-3 text-left transition hover:border-zinc-600"
            >
        ...
        {blocks.map((block, index) => {
          ...
          return (
            <ControlledDetails
              ...
              className="rounded-2xl border border-zinc-800 bg-black/35 p-3"
              defaultOpen={index === 0 || inlineWarnings.length > 0}
            >
              <summary className="flex cursor-pointer list-none items-center justify-between gap-3 [&::-webkit-details-marker]:hidden">
                <div className="flex min-w-0 items-center gap-3">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-zinc-800 bg-zinc-900 text-zinc-400">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <span className="text-xs text-zinc-500">Blok ayarları</span>
                <div className="flex gap-1.5">

...
function SectionBody({ section, index, slug, locale }: { section: EditorialSection; index: number; slug: CuratedPublicSlug; locale: PublicPageContent["locale"] }) {
  ...
  const livingStudies = slug === "karavan-deneyimi" && section.id === "sleep" ? ["alcove"] as const
    : slug === "karavan-deneyimi" && section.id === "personal-space" ? ["toilet", "shower"] as const
      : undefined;
  const hasVisual = Boolean(visual || isRoofEvaluation || livingStudies);
  return <section id={section.id} className={`sv-editorial-section ${hasVisual ? "sv-editorial-section-with-media" : "sv-editorial-section-text-only"} ${index % 2 ? "sv-editorial-section-reverse" : ""}`}>
    ...
    {visual ? <EditorialVisual visual={visual} locale={locale} /> : isRoofEvaluation ? <RoofEvaluation locale={locale} /> : livingStudies ? <PublicLivingConceptStudies copy={publicLaunchContent[locale].product} locale={locale} images={[...livingStudies]} /> : null}
  </section>;
}
...
export function PublicEditorialPage({ page, children }: { page: PublicPageContent; children?: React.ReactNode }): React.JSX.Element {
  ...
  return <main className={`sv-editorial sv-editorial-${page.slug}`}>
    ...
    {copy.faqGroups ? <div className="sv-container sv-editorial-faqs">
      ...
      {copy.sections.map((section, index) => slug === "karavan-deneyimi" && index === 0 ? null : <SectionBody key={section.id} section={section} index={index} slug={slug} locale={page.locale} />)}
    </div>}
